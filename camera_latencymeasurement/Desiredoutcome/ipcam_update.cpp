...
int main(int, char**) {
 ...
 const std::string videoStreamAddress = "http://root:root@192.168.0.95/axis-cgi/mjpg/video.cgi?resolution=640x480&req_fps=30&.mjpg";
 const std::string videoStreamAddressa = "http://root:root@192.168.0.96/axis-cgi/mjpg/video.cgi?resolution=640x480&req_fps=30&.mjpg";
  const std::string videoStreamAddressb = "http://root:root@192.168.0.97/axis-cgi/mjpg/video.cgi?resolution=640x480&req_fps=30&.mjpg";
   const std::string videoStreamAddressc = "http://root:root@192.168.0.98/axis-cgi/mjpg/video.cgi?resolution=640x480&req_fps=30&.mjpg";
 ...
 if(!vcapa.open(videoStreamAddressa)) {
        std::cout << "Error opening video stream or file1" << std::endl;
        return -1;
    }
     if(!vcapb.open(videoStreamAddressb)) {
        std::cout << "Error opening video stream or file2" << std::endl;
        return -1;
    }
     if(!vcapc.open(videoStreamAddressc)) {
        std::cout << "Error opening video stream or file3" << std::endl;
        return -1;
    }
   ...
   for(;;) {
	time_t tE = time(0);
        ...
        if(!vcap.read(image)) {}
	if(!vcapa.read(imagea)){} 
	if(!vcapa.read(imageb)){} 
	if(!vcapa.read(imagec)){} 
	...
	if(count_start==0)
	  ...
	  //cam 1
		object_recognition(image,name,event_id);
		 ...
		 //cam 2   
		    object_recognition(imagea,namea,event_ida);
		    if(first_timea==1){
		      event_prea=event_ida;
		      first_timea=0;}
		      if(event_ida!=event_prea){
		      event_prea=event_ida;
			TimerE(time_start,time_ona);
			//cout << endl<< setfill('0')<< setw(5) << event_pre  ;
			//cout <<  setfill(' ') <<  setw(15) << fixed    <<time_on <<endl  ;
			timeStampa.push_back(time_ona);
			ida.push_back(event_prea);
			publishData(  "cam2",  event_prea ,  time_ona);
		      }
		      
		    
		    
		    //cam3
		    object_recognition(imageb,nameb,event_idb);
		    if(first_timeb==1){
		      event_preb=event_idb;
		      first_timeb=0;}
		      if(event_idb!=event_preb){
		      event_preb=event_idb;
			TimerE(time_start,time_onb);
			//cout << endl<< setfill('0')<< setw(5) << event_pre  ;
			//cout <<  setfill(' ') <<  setw(15) << fixed    <<time_on <<endl  ;
			timeStampb.push_back(time_onb);
			idb.push_back(event_preb);
			publishData(  "cam3",  event_preb,  time_onb);
		      }
		      
		   
		    //cam4
		    object_recognition(imagec,namec,event_idc);
		    if(first_timec==1){
		      event_prec=event_idc;
		      first_timec=0;}
		      if(event_idc!=event_prec){
		      event_prec=event_idc;
			TimerE(time_start,time_onc);
			//cout << endl<< setfill('0')<< setw(5) << event_pre  ;
			//cout <<  setfill(' ') <<  setw(15) << fixed    <<time_on <<endl  ;
			timeStampc.push_back(time_onc);
			idc.push_back(event_prec);
			publishData(  "cam4",  event_prec,  time_onc);
		      }
		      
		     if( int(nowE->tm_hour)==thourE && int(nowE->tm_min) ==tminE && int(nowE->tm_sec) ==0){
			printlogfile( id ,timeStamp, logfile);
		        printlogfile( ida ,timeStampa, logfilea);
			 printlogfile( idb ,timeStampb, logfileb);
			  printlogfile( idc ,timeStampc, logfilec);
