...
bool trackblack(rectangles_Info* ROI,Mat COI){
	...
	for(int i=y;i < y1;i++){
		...
		//cout << "countBPixel    "<< countBPixel <<endl;
	if(countBPixel <100)
		return true;
	else return false;
...
// const std::string videoStreamAddress = "http://root:root@192.168.0.93/axis-cgi/mjpg/video.cgi?resolution=640x480&req_fps=30&.mjpg";
	...
	//else {
	  //cam 1
		object_recognition(image,name,event_id);
		    if(first_time==1){
		      event_pre=event_id;
		      first_time=0;} //event_id-event_pre<2 
