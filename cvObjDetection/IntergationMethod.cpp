...
void surfMatching(Mat img_1,Mat img_2){
...
imwrite("goodmatch.jpg",img_matches);

}

void xmlDetection(Mat frame,string img, string xmlFile,float a,float b){
    CascadeClassifier cascade;
    const float scale_factor(1.2f);
    const int min_neighbors(24);
	cascade.load(xmlFile); 
	Mat frameTemp, img_gray ;;
 	Mat input_image =  imread(img, CV_LOAD_IMAGE_COLOR);
 	 Size size(960,720);
 	resize(input_image,input_image,size);
	...
	cout <<reject_levels[n]<< "\t\t" << level_weights[n] <<endl;
		if(reject_levels[n]>a  && level_weights[n] >b){
			...
			surfMatching(frameROI,input_image);
...
int main(int argc, char **argv) {
	...
	Mat frame; 
      for(;;){
	    ...
	    cap >> frame;
            xmlDetection( frame,"SensorB.jpg", "SensorB.xml",stoi(argv[1]),stoi(argv[2]));
